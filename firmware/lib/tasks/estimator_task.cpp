...
static void publish_state(FcQueues *q, const fc_state_estimate_t *state) {
...
void estimator_task(void *pv) {
    EstimatorTaskConfig *cfg = (EstimatorTaskConfig *)pv;

    fc_attitude_filter_t attitude;
    fc_attitude_filter_init(&attitude, MADGWICK_BETA, /*have_mag=*/1);
        ...
        if (sample.type == RAW_SAMPLE_BARO) {
            if (!have_reference) {
                ...
                if (pressure_count >= REFERENCE_CALIB_SAMPLES) {
                    ...
                    have_reference = true;
                    log_i("estimator_task: pad reference pressure: %.1f Pa", reference_pressure_pa);
                }
                continue;
            }
            /* International barometric formula, pad-relative altitude. */
            float baro_alt_m = 44330.0f * (1.0f - powf(sample.as.baro.pressure_pa /
                                                             reference_pressure_pa, 0.1903f));
            fc_altitude_filter_update_baro(&altitude, baro_alt_m);
        } else if (sample.type == RAW_SAMPLE_GPS) {
            /* GPS altitude is MSL, not pad-relative -- without a separate
             * pad GPS-altitude calibration this would bias the filter, so
             * GPS altitude fusion is intentionally left disabled for now.
             * Position (lat/lon) still flows to telemetry via
             * queues->gps_to_telemetry (see sensor_task.cpp). */
            continue;
             ...
             * matches the LSM9DS1's sign (reaction-force positive) --
